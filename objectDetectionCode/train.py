'''
This file is dedicated to the traning phase of a model. You can start from
scratch or save the model for each epoch. You can also evalaute the model once
in a while or for each iteration. Several parameters and hyperparameters can be
specified.

PyTorch Detection Training.

To run in a multi-gpu environment, use the distributed launcher (not really tested):
...
def main(args):
    ...
    # By default validation after each epoch is performed on the easy testing subset
    dataset, num_classes = get_dataset(args.dataset, "train", get_transform(train=True), args.data_path) #train
    dataset_test, _ = get_dataset(args.dataset, "val_easy", get_transform(train=False), args.data_path)  #validate
    ...
    # Replace the pre-trained head with a new one (to match the AVD classes)
    ...
    # strategy choice for learning rate decay 
    # lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.lr_step_size, gamma=args.lr_gamma) 
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=args.lr_steps, gamma=args.lr_gamma)

    if args.resume: #to resume the training process from a checkpoint
